...
def stoGradecent(X,Y,W,N):
    ...
    #return there3
    #print (-np.dot(X[N,:],W))
    return -(X[N,:]*sigmoid((-np.dot(X[N,:],W))[0]*Y[N][0])*Y[N][0]).reshape((21,1))   #一个bug调了一晚上，果然矩阵运算很容易错，错基本都是行列出的错，这个改啊......
...
yita=0.001
for i in range(2000):
    W=W-yita*stoGradecent(train_X,train_Y,W,i%1000)
    #print(W)
